...
function renderV2Hazard(){const box=document.getElementById('v2Hazard');if(!box)return;const niv=HAZ_NIVELES;
  ...
  const NIV_DOT={'Muy alto':'#8f0a0a','Alto':'#d21f1f','Medio':'#e8624a'};
  h+=`<div class="filter-label">Nivel compuesto</div>`+chkBar('hn')+`<div class="checks">`+niv.map(v=>`<label><input type="checkbox" data-v2hn="${escapeAttr(v)}" ${v2.hazNivel.has(v)?'checked':''}><span class="chip-dot" style="background:${NIV_DOT[v]||'#999'}"></span>${escapeHtml(v)}</label>`).join('')+`</div>`;
